...
CNVStateProcess <- function(HMMcopyReadFile, RemoveArtifactCell=TRUE, BinRegionDelete=TRUE, MySampleID="MySampleID") {
      ...
      #   table(is.na(x))[names(table(is.na(x)))=="TRUE"]
       ...
       Median_MAD_GiniCoef <- median(GiniCoefficient)+ mad(GiniCoefficient); print(Median_MAD_GiniCoef)  # 0.1705 # 0.02905896 # 0.199559
       ...
       length(CellID_ToRemoveByGini); CellID_ToRemoveByGini[1:3]; # pt1 254 #  "X01_040_143929A_R04_C18" "X01_040_143929A_R07_C36" "X01_040_143929A_R10_C11"
       ...
       table(CellID_ToRemoveByMean %in% CellID_ToRemoveByGini) # TRUE 114
       CellID_ToRemoveByMeanGini <- unique(c(CellID_ToRemoveByMean, CellID_ToRemoveByGini)); length(CellID_ToRemoveByMeanGini) # Pt1. 511 #
       ...
       dim(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini); HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini[1:2,1:3] # Pt1. 698 5073 Pt2. 886 5082  #Pt3. 705 4922
      ...
      #### +++++++++++ #### +++++++++++ #### +++++++++++ #### +++++++++++ #### +++++++++++ #### +++++++++++ #### +++++++++++ #### +++++++++++
       if(RemoveArtifactCell==FALSE & BinRegionDelete==TRUE) {
         ...
         if(BinRegionDelete==TRUE) {
             ...
             ColNameProcess <- gsub("_(.*)","",colnames(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini)); length(ColNameProcess) # 5475
             table(duplicated(ColNameProcess)) ## FALSE 23  TRUE 5236
             ColNameProcess[duplicated(ColNameProcess)] <- ""

             HMMcopy_StateIdeal_RmvArtifactCNV_WithoutBinRegion <- HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini
             colnames(HMMcopy_StateIdeal_RmvArtifactCNV_WithoutBinRegion) <- ColNameProcess

             saveRDS(HMMcopy_StateIdeal_RmvArtifactCNV_WithoutBinRegion, file=paste0("HMMcopy_StateIdeal_RmvCellByGiniMean_WithoutBinRegionAnnot_", MySampleID, ".rds"))
             return(HMMcopy_StateIdeal_RmvArtifactCNV_WithoutBinRegion)
         } else {
             return(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini)
         }
       }
}
...
HierarchialClusteringHeatmap <- function(Processed_HMMcopyStateData=HMMcopy_StateIdeal_RmvArtifactCNV_WithoutBinRegion,  MySampleID="MySampleID",
  ...
  RowSplitHeatmap_Cluster <- ComplexHeatmap::Heatmap(as.matrix(Processed_HMMcopyStateData), col=col_fun_CNV, show_row_names=FALSE,show_column_names=TRUE,
...
#' Title RemoveArtifactCellHeatmap
#'
#' @param CNVStateData_AfterRmvHighGiniMeanFile   Output of step2.
#' @param MySampleID    Manually typed in
#' @param KM_ClusterSize    cluster size that I want
#' @param ThresholdLowState   bins of low state are bigger than 20%
#' @param ThresholdHighState   bins of high state are more than 10
#'
#' @return make CompHmapCNV_RmvByGiniMeanLowHighState_AllChr pdf file
#' @export
#'
#' @examples RemoveArtifactCellHeatmap(CNVStateData_AfterRmvHighGiniMeanFile="HMMcopy_StateIdeal_RmvCellByGiniMean_WithoutBinRegion_01_213_143939A.rds",MySampleID, KM_ClusterSize=9, Chr1_KM_ClusterSize=5, ThresholdLowState)
RemoveArtifactCellHeatmap <- function (CNVStateData_AfterRmvHighGiniMeanFile="HMMcopy_StateIdeal_RmvCellByGiniMean_WithoutBinRegion_01_213_143939A.rds",
                                              MySampleID, KM_ClusterSize=9,  ThresholdLowState=0.2, ThresholdHighState=10) {
      ...
      ########## ++++++++++ Remove artifacts by cluster number of cells.
      # HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc <-  RemoveCell_ByClusterNumb(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc, HierarchialClusterInfoFile, ClusterNumbToRmv, ClusterNumbToPick=NULL )

      ##### +++++++++++++++++ Compare Cell IDs between after removing by MeanState + Geni Coef  and By Cluster 6789
      print(paste0("Number of cells after removing artifact by clusters: ", nrow(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc) ) )

      if(is.null(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc)) {
      } else {
          print(paste0("Number of cells after removing artifact by high Gini and Mean: ", nrow(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc) ))
          CommonCellTable <- table(rownames(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc) %in% rownames(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc))

          print(CommonCellTable[names(CommonCellTable)==TRUE])

          # cat(paste0(MySampleID,"\n"), file="CountCommonCell_Btw_RmvArtifact_ByCluster_ByGiniMean.txt")
          # cat(
          #     print(paste0("number of common cells between two artifact removal, Out of ", nrow(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc), " cells after removing artifacts by cluster, ",
          #                  CommonCellTable[names(CommonCellTable)==TRUE], " cells are common.")),
          #     file="CountCommonCell_Btw_RmvArtifact_ByCluster_ByGiniMean.txt", append=TRUE)
      }
      #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++
      #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++
      ...
      Count_LowState01 <- apply(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc, 1, function(x) {
      ...
      length(Count_LowState01) # 881 cells

      ### count the number of bins with state > 5
      Count_HighState5_10 <- apply(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc, 1, function(x) {
      ...
      length(Count_HighState5_10) # 881 cells

      ## 20% bins of ncol(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc)
      NumbCellLowState <- ncol(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc)*ThresholdLowState; # ## Originaly, I used "*0.15" print(NumbCellLowState)
      ## Cell IDs to remove
      CellID_LowStateToRmv <- names(Count_LowState01)[Count_LowState01 >= NumbCellLowState];
      print(paste0("number of cell IDs to remov by Low State: ", length(CellID_LowStateToRmv))) # 176

      ## Find cells of High State > 10 bins. # default is 10
      # ThresholdHighState <- 10 #  ncol(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc)*0.15; #  print(NumbCellLowState)
      ## Cell IDs to remove
      ...
      RowSum_CNVState <- rowSums(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc)
      ...
      print(paste0("Number of cells rowSums > 13000: ", length(CellID_HighRowSumStateToRmv) ))

      ## remove cells of low or high CNV state, or HighRowSum State
      HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState <- HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc[!(rownames(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc) %in%
                                                                                                 c(CellID_LowStateToRmv, CellID_HighStateToRmv, CellID_HighRowSumStateToRmv)),]
      dim(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState) # 324 5259
      Numb_Cell <- nrow(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState)
      print(paste0("After remove outlier cells of Low or High CNV state: ", nrow(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState)))
      saveRDS(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState, paste0("HMMcopy_StateIdeal_RmvCellByGiniMean","_", MySampleID,"_LowHighState",
                                                                                          ThresholdLowState, ".rds"))

      #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++ #### +++++++++++++++++
      col_fun_CNV = circlize::colorRamp2(c(0:10),
                              c( "#2EA3DE",    "#72C5EF","#D5E3EB","#FFCC99","lightsalmon","#FF9933",    "darkorange","red1","red2","#CC0066", "#99004C"))
      RowSplitHeatmap_RmvCluster6789 <- ComplexHeatmap::Heatmap(as.matrix(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState), col=col_fun_CNV,
                                                                show_row_names=FALSE,show_column_names=TRUE, cluster_columns=FALSE, cluster_rows=TRUE,  row_km=KM_ClusterSize, use_raster=TRUE)
      pdf(paste0("CompHmapCNV_RmvByGiniMeanLowHighState_AllChr_", MySampleID,"_LowSt",ThresholdLowState,"_", Numb_Cell, "Cells_",KM_ClusterSize,"clu.pdf"), width=10,height=10)  # height=3
            RowSplitHeatmapDraw_RmvCluster6789 <- ComplexHeatmap::draw(RowSplitHeatmap_RmvCluster6789)
      ...
      ##### +++++++++   =============       ##### +++++++++   ============= ###      ##### +++++++++   ============= ###
      #### Remove cluster with cells of the number < 15
      RowIndexExtract <- ComplexHeatmap::row_order(RowSplitHeatmapDraw_RmvCluster6789); length(RowIndexExtract) # 9
      ...
      ## Pt. 5
      #   1   9   8   7   6   5   2   3   4
      #   36   2  13  35   9 147  74  22   6
      ## Pt. 8
      # 5   2   3   4   1
      # 15  26 334 125  27

      ## Find which cluster has the elements < 15
      ...
      saveRDS(RowIndexExtract_NoLowCellCluster, file=paste0("RowSplitHeatmapDraw_WithoutArtifactLowHighState_AllChr_",MySampleID,"_",KM_ClusterSize,"clu.rds"))  # problem is here.

      #####################################       #####################################       #####################################       #####################################
      #####################################       #####################################       #####################################       #####################################
      ###  ####. Extract cells in each cluster and stack in order.  Extract only chr01 bins.     ### I will not make a heatmap of Chr1 only here. I will do it in step 5.


      # RowIndexExtract <- ComplexHeatmap::row_order(RowSplitHeatmapDraw_RmvCluster6789); length(RowIndexExtract) # 9
      # # RowIndexExtract$"1" # [1] 323 329 246 327 245 334 374 324 248 259 371 258 268 157 267 336 251 373 328 375 254 322 152 252  58  76 253 270 261
      # # RowIndexExtract$"5" #  310 317
      # # Cluster5Cell <- rownames(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState[RowIndexExtract$"5", ])
      # # table(Cluster5Cell %in% CellID_HighRowSumStateToRmv)
      # # RowIndexExtract$"4" #  [1]  11  30 320   3 113 315 365 192 220 106 351 173 207
      # table(t(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowState[11,])) # cluster 4
      # table(t(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowState[323,])) # cluster 1 top
      # table(t(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowState[265,])) # cluster 1 middle
      #
      # ## 2nd round
      # RowIndexExtract$"2" # [1] 311 308 251 153 245 250 241
      # table(t(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState[311,])) # cluster 1 middle

      # RowIndexExtract_old <- RowIndexExtract
      # CellID_LowStateOld <- c(RowIndexExtract_old$"1", RowIndexExtract_old$"2")
      # table(CellID_LowStateToRmv %in% rownames(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc)[CellID_LowStateOld] )

      LoopNumb <- 0; HMMcopy_StateIdeal_SubsetCluster<-data.frame()
      for (EachClusterNumber in RowIndexExtract) {
          # EachClusterNumber <- RowIndexExtract$'1';  print(EachClusterNumber)
          LoopNumb <- LoopNumb+1;

          ## Select only Chromosome1 bins
          BinPosition_Chr02 <- grep("chr02", colnames(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState)); #print(BinPosition_Chr02) # 371
          HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_Chr01 <- HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_RmvLowHighState[, 1:(BinPosition_Chr02-1)]; dim(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_Chr01) # 324 370

          HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_SubsetCluster <- HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_Chr01[EachClusterNumber, ]; dim(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_SubsetCluster) # 103 429
          # rownames(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_SubsetCluster) <- paste0("Cluster", names(RowIndexExtract[LoopNumb]))

          HMMcopy_StateIdeal_SubsetCluster <- rbind(HMMcopy_StateIdeal_SubsetCluster, HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc_SubsetCluster)
      }
      dim(HMMcopy_StateIdeal_SubsetCluster) # 532 426
      #saveRDS(HMMcopy_StateIdeal_SubsetCluster, file=paste0("HMMcopy_StateIdeal_RmvCellByGiniMean_chr1Only_", MySampleID,"_LowState",ThresholdLowState, ".rds"))

      RowSplitHeatmap_SubsetCluster <- ComplexHeatmap::Heatmap(as.matrix(HMMcopy_StateIdeal_SubsetCluster), col=col_fun_CNV,
                                                               show_row_names=FALSE,show_column_names=FALSE, cluster_columns=FALSE, cluster_rows=FALSE, use_raster=TRUE)
      #pdf(paste0("CompHeatmap_CNV_RmvByGiniMeanCluster_OnlyChr01_", MySampleID, "_LowSt",ThresholdLowState, ".pdf"), width=10,height=10)  # height=3
      #        RowSplitHeatmapDraw_SubsetCluster <- ComplexHeatmap::draw(RowSplitHeatmap_SubsetCluster)
      #dev.off()

      ############       ############       ############       ############       ############       ############       ############
      ## ++++++++ Just try to cluster chr1 segements by a given cluster size. +++++++++++ ======
      # RowSplitHeatmap_SubsetCluster_chr1 <- ComplexHeatmap::Heatmap(as.matrix(HMMcopy_StateIdeal_SubsetCluster), col=col_fun_CNV,
      #                                  show_row_names=FALSE,show_column_names=FALSE, cluster_columns=FALSE, cluster_rows=FALSE, use_raster=TRUE, row_km=Chr1_KM_ClusterSize)
      # pdf(paste0("CompHeatmap_CNV_RmvByGiniMeanCluster", ClusterNumbConcat,"_ClusterOnlyChr01_", MySampleID, ".pdf"), width=10,height=10)  # height=3
      #         RowSplitHeatmapDraw_SubsetCluster_chr1 <- ComplexHeatmap::draw(RowSplitHeatmap_SubsetCluster_chr1)
      # dev.off()
      # saveRDS(RowSplitHeatmapDraw_SubsetCluster_chr1, file=paste0("RowSplitHeatmapDraw_WithoutArtifact_OnlyChr1_ByClusterSize", Chr1_KM_ClusterSize, "_", MySampleID,".rds"))

      ### ==== #8, 09_025_143855A  has CNA loss at the end of Chr1p arm. Chr1p: 1 - 123,400,000  Chr1q: 123,400,001 - 248,956,422
      ColMean <- colMeans(HMMcopy_StateIdeal_SubsetCluster); length(ColMean) # 426
      names(ColMean) <- 1:length(ColMean)
      # plot(x=names(ColMean), y=ColMean)
      names(ColMean)[ColMean<1.5] # [1] "233" "234" "235" "236"
...
RemoveReorderCell_ByClusterNumb <- function(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc, HierarchialClusterInfoFile, ClusterNumbToRmv=NULL,
    ...
    if(is.null(ClusterNumbToPick) & !is.null(ClusterReorder)) { ## This is reordering.
        ...
        HMMcopy_State_ClusterToReordered <- HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc[IndexCell_ReorderClusterAll,]; dim(HMMcopy_State_ClusterToReordered)  # 387 5259
...
RemoveNormalClusterReorderHeatmap <- function (HierarchialClusterInfoFile="RowSplitHeatmapDraw_WithoutArtifactLowHighState6_AllChr_01_213_143839A.rds",
      ...
      dim(HMMcopy_StateIdeal_AfterRmvArtifactCell_ColNameProc) #  474 5259
      ...
      dim(HMMcopy_StateIdeal_AfterRmvArtifactCell_RmvReorderCluster) # 387 5259
      ...
      Position_chrStart <- grep("chr", colnames(HMMcopy_StateIdeal_AfterRmvArtifactCell_RmvReorderCluster)); length(Position_chrStart) ; print(Position_chrStart) ## 24
              ...
              ComplexHeatmap::decorate_heatmap_body("CloneHeatmap", {
                ...
                for (i in 1:(length(Position_chrStart) +1) ) {
                  ...
                  # grid::grid.lines(c( EndChrPositionPercent, EndChrPositionPercent), c(-nrow(HMMcopy_StateIdeal_AfterRmvArtifactCell_RmvReorderCluster)/4, 1), gp = grid::gpar(col = "white", lwd = 1)) # c(-20, 1) <= c(bot, top)
                   grid::grid.lines(c( EndChrPositionPercent, EndChrPositionPercent), c(-25, 1), gp = grid::gpar(col = "white", lwd = 1)) # c(-20, 1) <= c(bot, top)
                }
              } )
...
#'
#' @param HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_File   HMMcopy data after remove artifact cells
#' @param HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorderFile    HMMcopy data after remove artifact cells and after reordered
#' @param MySampleID   Manually typed in.
#'
#' @return CompHeatmap_Chr1CentromereRegion_ pdf file
#' @export
#'
#' @examples Chr1Centromere_Heatmap(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_File="HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_01_213_143839A.rds", HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorderFile="HMMcopy_StateIdeal_AfterRmvArtifactCell_RmvReorderCluster_01_213_143839A.rds", MySampleID="01_213_143839A")
Chr1Centromere_Heatmap <- function (HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_File, HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorderFile, MySampleID) {

      HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini <- readRDS(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_File)
      dim(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini) # 637 5259

      HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorder <- readRDS(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorderFile)
      dim(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorder) # Pt#5.  314 5259

      #### Select only cells filtered in by HighGiniMean, StateLowHigh and ClusterRmv  ==> reorder to match with "HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorderFile"
      HMMcopy_StateIdeal_RmvbyGiniMeanStateLowHigh <- dplyr::filter(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini, rownames(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini) %in%
                                                                               rownames(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorder))
      dim(HMMcopy_StateIdeal_RmvbyGiniMeanStateLowHigh) # 387 5259

      CellOrderIndex <- order(rownames(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorder))
      HMMcopy_StateIdeal_RmvbyGiniMeanStateLowHigh_Reorder <- HMMcopy_StateIdeal_RmvbyGiniMeanStateLowHigh[order(CellOrderIndex),]
      ### Checking the reorderd cell IDs
      identical(rownames(HMMcopy_StateIdeal_RmvbyGiniMeanStateLowHigh_Reorder), rownames(HMMcopy_StateIdeal_RmvArtifactCNV_ByMeanGini_StateLowHighReorder)) # TRUE
      ...
      # colnames(HMMcopy_StateIdeal_RmvbyGiniMeanStateLowHigh_Reorder)[233:236] ## Pt8 # "chr01_145500001_146000000" "chr01_146500001_147000000" "chr01_147000001_147500000" "chr01_147500001_148000000"
...
Karyoplote_BySegment <- function(CNVSegmentFile="/Users/lees130/Library/CloudStorage/OneDrive-NYULangoneHealth/N07_DLPplus_scWGS/04_DLPplus_1qGain_8ptData/01-213_143839A/hmmcopy_segments.csv.gz",
    ...
    CNVSeg_StateIdealSelect_Proc[1:2,]; dim(CNVSeg_StateIdealSelect_Proc) ; length(unique(CNVSeg_StateIdealSelect_Proc$cell_id))# 5357653       5
    #           chr start     end state   median                 cell_id
    #        <char> <int>   <int> <int>    <num>                  <char>
    ...
    dim(CNVSeg_StateIdeal_AfterRmvArtifactCell_ColNameProc); length(unique(rownames(CNVSeg_StateIdeal_AfterRmvArtifactCell_ColNameProc))) # 242
    #                         chr01   .1 .2 .3
    ...
    if(is.null(CNVSeg_StateIdeal_AfterRmvArtifactCell_ColNameProc)) {
        ...
        dim(CNVSeg_StateIdealSelect_RmvByGiniMeanState); length(unique(CNVSeg_StateIdealSelect_RmvByGiniMeanState$cell_id)) # 111534       5 # 242

    }
    NumbOfCell <- length(unique(CNVSeg_StateIdealSelect_RmvByGiniMeanState$cell_id)); print(paste0("Numb of cells: ",NumbOfCell)) # 242
    ...
    pdf(paste0("OutKaryoplote_AllCell_BeforeRmvHighGiniMean_BySegMedian_ByCluster",ClusterNumbConcat,"_", MySampleID, "_PloidyLine",NumbOfCell,"c.pdf"), height=6,width=12)
        ...
        plotLRR(kp, CNVSeg_StateIdealSelect_RmvByCluster_Tiles, ymin=0, ymax=5, add.axis = FALSE, labels=NA)
